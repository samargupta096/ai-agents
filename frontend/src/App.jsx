...
import { checkOllamaConnection, fetchOllamaModels, generateWithOllama, formatDuration, calculateTokensPerSecond } from './utils/ollama'
...
function App() {
  ...
  useEffect(() => {
    const checkConnections = async () => {
      ...
      try {
        const res = await fetch(`${BACKEND_API}/`, { signal: AbortSignal.timeout(3000) })
  ...
  const fetchAgents = async () => {
    try {
      ...
      const fallbackAgents = [
        { id: 'generate', name: 'Code Generator', description: 'Generate production-ready code from natural language descriptions', icon: '💻', color: '#3B82F6' },
        { id: 'review', name: 'Code Reviewer', description: 'Review code for bugs, security issues, and best practices', icon: '🔍', color: '#10B981' },
        { id: 'debug', name: 'Bug Detective', description: 'Debug issues and find root causes of errors', icon: '🐛', color: '#EF4444' },
        { id: 'test', name: 'Test Generator', description: 'Generate comprehensive unit and integration tests', icon: '🧪', color: '#8B5CF6' },
        { id: 'docs', name: 'Doc Writer', description: 'Generate documentation, docstrings, and README files', icon: '📚', color: '#F59E0B' },
        { id: 'refactor', name: 'Refactoring Expert', description: 'Suggest code improvements and refactoring opportunities', icon: '⚙️', color: '#EC4899' },
      ]
  ...
  const buildPrompt = (customInput = input, customCode = codeContent, customError = errorMessage) => {
  ...
  const executeAgent = async () => {
    ...
    try {
      if (settings.useDirectOllama && ollamaStatus === 'connected') {
    ...
    for await (const chunk of stream) {
      if (chunk.type === 'token') {
        ...
        stepMetadata = {
          ...
          tokensPerSecond: calculateTokensPerSecond(meta.generatedTokens, meta.evalDuration),
          contextUsed: `${meta.promptTokens + meta.generatedTokens}/${settings.contextWindow}`,
        }
      }
    }

    return { output: fullOutput, metadata: stepMetadata, duration: Date.now() - startTime }
  }
  ...
  const executeWorkflow = async (workflow) => {
    ...
    try {
      for (let i = 0; i < workflow.length; i++) {
        ...
        setWorkflowOutputs([...allOutputs])
        setMetadata(result.metadata)
      }

      // Combine all outputs for final display
      const combinedOutput = allOutputs.map((o, i) => 
        `## Step ${i + 1}: ${o.agent.icon} ${o.agent.name}\n\n${o.output}`
      ).join('\n\n---\n\n')
      
      setOutput(combinedOutput)
      setStatus('✅ Workflow Complete')
      showToast(`Workflow completed! ${workflow.length} steps executed.`, 'success')
  ...
  const needsCodeInput = selectedAgent && ['review', 'debug', 'test', 'docs', 'refactor'].includes(selectedAgent.id)
